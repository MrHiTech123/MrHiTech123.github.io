...
function draw() {
    ...
    if (in_spellbook) {
        ...
        fill(250, 180, 30)
        text(spellbook_in_script, screen.width / 2, 75)
        textSize(30);
        text('Press enter to continue to level, "d" for documentation.', screen.width / 2, 150)
        ...
        for (i in spellbook) {
            ...
            rect(450 + 150 * Math.floor(i / 5), 180 + 100 * (i % 5), 150, 100)
            if (i == current_spell_index) {
                fill(0, 0, 255);
            }
            else {
            ...
            text(spellbook[i], 450 + 150 * Math.floor(i / 5), 230 + 100 * (i % 5), 150)
        }
        if (current_keyboard != -1) {
            text('Press esc to return to parent menu', 160, 665)
        }
        ...
        for (i in currtypes) {
            ...
            text('Press ' + ((parseInt(i) + 1).toString().slice(-1)), 60 + 75 * i, 700);
